...
More information about the archiving project can be found on the ArchiveTeam wiki: [codeblender](http://archiveteam.org/index.php?title=codeblender)
...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "codeblender" project in the Warrior interface.
...
### For Debian/Ubuntu:
    ...
    apt-get update && apt-get install -y git-core libgnutls-dev lua5.1 liblua5.1-0 liblua5.1-0-dev screen python-dev python-pip bzip2 zlib1g-dev flex
    pip install --upgrade seesaw
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/codeblender-grab.git; cd codeblender-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/codeblender-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]

In __Debian Jessie__, the `libgnutls-dev` package was renamed to `libgnutls28-dev`. So, you need to do the following instead:

    adduser --system --group --shell /bin/bash archiveteam
    apt-get update && apt-get install -y git-core libgnutls28-dev lua5.1 liblua5.1-0 liblua5.1-0-dev screen python-dev python-pip bzip2 zlib1g-dev flex
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, codeblender-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/codeblender-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
